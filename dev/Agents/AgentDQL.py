...
class AgentDQL:
    ...
    def plot_history(self, grid=False):
        ...
        plt.subplot(2, 1, 2)
        plt.plot(range(len(self.rewards_hist)), self.rewards_hist)
        plt.xlabel('episode')
        plt.ylabel('reward')
...
# uses the simulated environment with a differnt state
class AgentDCQL(AgentDQL):
    def create_network(self):
        ...
        model = keras.Sequential([
            keras.layers.Conv2D(24, (2, 2), activation='relu', input_shape=(self.env.n_lin, self.env.n_col, 2)),
            keras.layers.MaxPooling2D((2, 2)),
            keras.layers.Conv2D(18, (2, 2), activation='relu'),
    ...
    def learning(self):
        ...
        for i in range(self.episodes):
            ...
            self.env.regenerate()
            self.s = self.env.to_state([0, 0])

            duration_limit = 100

            while not self.env.is_final(self.s) and duration_limit > 0:
                ...
                def fformat(m):
                    sss = np.argmax(m)
                    return [sss // self.env.n_col, sss % self.env.n_col]


                #print("res:", reward, a, fformat(self.s[:,:,0]), fformat(next_state[:,:,0]), fformat(next_state[:,:,1]), [[-1, 0], [0, 1], [1, 0], [0, -1]][a])
    ...
    def replay(self, i):
        ...
        for m in memory_samples:
            ...
            target = self.target_network.predict(self.state_to_input(state))
            if self.env.is_final(new_state):
                target[0][action] = reward
            else:
                target[0][action] = reward + self.gamma * max(self.target_network.predict(self.state_to_input(new_state))[0])
